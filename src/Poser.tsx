...
export const Poser = ({ url, onInteractingChanged }: PoserProps) => {
    ...
    const editorWidth = width * .9;

    return <>
        <Avatar url={url} position={[0, -1, 0]} pose={currentPose} />
        <Hud renderPriority={4}>
            <OrthographicCamera makeDefault position={[0, 0, 100]} />
            <PoserHud width={editorWidth} height={editorHeight} position={[0, offsetY, 0]} onTimeChange={_ => setCurrentTime(_)}
                onAnimationChange={_ => setAnimation(_)} onPointerDown={_ => {
